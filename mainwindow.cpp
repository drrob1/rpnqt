...
//    PushStacks();  looks like there were too many calls to PushStacks();  When I commented this out, undo started to work.
    ...
    } else if (cmdstr.compare("p") EQ 0) {
        // do nothing, but don't feed cmdstr into GetResult.
    ...
    } else if (cmdstr.find("sho") EQ 0) {
        string str = to_string(SigFig);
        QString QS = QString::fromStdString(" sigfig= " + str);
        ui->listWidget_Output->addItem(QS);
        str = to_string(OutputState);
        QS = QString::fromStdString("OutputState= " + str);
        ui->listWidget_Output->addItem(QS);
